...
class GameEngine {
    ...
    createInitialGameState(roomId, boardSize) {
        ...
        return {
            ...
            winner: null,
            boardSize,
            maxDotsPerCell: shared_1.GAME_CONFIG.MAX_DOTS_PER_CELL,
            initialDotsPerPlayer: shared_1.GAME_CONFIG.INITIAL_DOTS_PER_PLAYER
        };
    }
    addPlayer(playerId, playerName) {
        if (this.gameState.players.length >= shared_1.GAME_CONFIG.MAX_PLAYERS) {
    ...
    startGame() {
        if (this.gameState.players.length < shared_1.GAME_CONFIG.MIN_PLAYERS) {
    ...
    processChainReactions(position, playerId, reactions) {
    ...
    getNeighbors(position) {
        ...
        // Down
        if (row < this.gameState.boardSize.rows - 1)
            neighbors.push({ row: row + 1, col });
        // Left
        if (col > 0)
            neighbors.push({ row, col: col - 1 });
        // Right
        if (col < this.gameState.boardSize.cols - 1)
            neighbors.push({ row, col: col + 1 });
        return neighbors;
    }
    updatePlayerDotCounts() {
